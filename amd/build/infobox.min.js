...
/**
   ...
   */Object.defineProperty(_exports,"__esModule",{value:!0}),_exports.renderInfoBox=void 0,Templates=_interopRequireWildcard(Templates),config=_interopRequireWildcard(config);_exports.renderInfoBox=async function(component,userId,selectorOrElement){let purposes=arguments.length>3&&void 0!==arguments[3]?arguments[3]:[];const targetElement=selectorOrElement instanceof Element?selectorOrElement:document.querySelector(selectorOrElement),aiInfoUrl=new URL(config.wwwroot+"/local/ai_manager/ai_info.php");purposes.forEach((purpose=>{aiInfoUrl.searchParams.append("purposes[]",purpose)}));const templateContext={aiinfourl:aiInfoUrl},{html:html,js:js}=await Templates.renderForPromise("local_ai_manager/infobox",templateContext);if(Templates.prependNodeContents(targetElement,html,js),window.innerWidth<=576){const textElement=targetElement.querySelector("p");textElement.classList.add("local_ai_manager-expandable_text"),textElement.addEventListener("click",(()=>{textElement.classList.toggle("local_ai_manager-expanded")}))}}}));
