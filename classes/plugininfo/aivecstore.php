...
/**
 * Plugininfo class for the subplugintype aivecstore.
 *
 ...
 */
class aivecstore extends base {
    #[\Override]
    public static function get_enabled_plugins() {
        global $DB;

        $plugins = core_plugin_manager::instance()->get_installed_plugins('aivecstore');
        if (!$plugins) {
            return [];
        }
        $installed = [];
        foreach ($plugins as $plugin => $version) {
            $installed[] = 'aivecstore_' . $plugin;
        }
    ...
    public function load_settings(\part_of_admin_tree $adminroot, $parentnodename, $hassiteconfig) {
        ...
        if ($adminroot->fulltree) {
            $shortsubtype = substr($this->type, strlen('aivecstore'));
